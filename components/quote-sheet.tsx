...
export function QuoteSheet({ quote, className }: { quote: Quote; className?: string }) {
  ...
  return (
    ...
    >
      ...
      <table className="mt-8 w-full border-collapse">
        <thead>
          <tr className={cn('border-b text-[10px] tracking-wider uppercase', t.headRule, t.muted)}>
      ...
      <div className="mt-4 flex justify-end">
        <div className="w-64 space-y-1 text-[12px]">
          ...
          {totals.discount > 0 && (
            <div className="flex justify-between">
              <span className={t.muted}>Descuento ({quote.discountPct}%)</span>
              <span className="tabular-nums">- {money(totals.discount)}</span>
            </div>
          )}
          {totals.tax > 0 && (
            <div className="flex justify-between">
              <span className={t.muted}>IVA / impuestos ({quote.taxPct}%)</span>
      ...
      {quote.notes && (
        <div className="mt-10">
          ...
          <p className={cn('mt-1 text-[12px] whitespace-pre-line', t.muted)}>{quote.notes}</p>
        </div>
      )}
    </div>
  );
